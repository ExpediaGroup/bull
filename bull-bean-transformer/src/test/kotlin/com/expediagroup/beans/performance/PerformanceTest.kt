...
class PerformanceTest {
    ...
    private fun initObjects() {
        ...
        IntStream.range(0, TOTAL_SIMPLE_OBJECTS).parallel().forEach { i: Int -> (sourceFooSimpleList as ArrayList<String>).add(i.toString()) }
        fromSubFooList = ArrayList()
        IntStream.range(0, TOTAL_COMPLEX_OBJECTS).parallel().forEach { i: Int -> (fromSubFooList as ArrayList<FromSubFoo?>).add(fromSubFoo) }

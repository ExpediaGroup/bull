...
/**
 * Unit test for [IntegerConversionProcessor].
 */
class IntegerConversionTest : AbstractConversionTest() {
    /**
     * The class to be tested.
     */
    @InjectMocks
    private val underTest: IntegerConversionProcessor? = null
    ...
    fun testConvertByteShouldReturnProperResult() {
        ...
        // THEN
        Assertions.assertThat(actual).isEqualTo(INTEGER_VALUE)
    }

    @Test
    fun testConvertByteArrayShouldReturnProperResult() {
        // GIVEN
        val expected = ByteBuffer.wrap(EIGHT_BYTE_BYTE_ARRAY).int
    ...
    /**
     * Tests that the method `convertBoolean` returns the expected int.
    ...
    private fun booleanToIntConvertValueTesting(): Array<Array<Any>> {
        return arrayOf(
            arrayOf(
                ...
                BOOLEAN_VALUE,
                TRUE_AS_INT
            ), arrayOf("Tests that the method returns 0 if the value is false", java.lang.Boolean.FALSE, FALSE_AS_INT)
        )
    }

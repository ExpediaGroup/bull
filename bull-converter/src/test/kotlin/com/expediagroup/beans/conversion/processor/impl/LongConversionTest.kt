...
/**
 * Unit test for [LongConversionProcessor].
 */
class LongConversionTest : AbstractConversionTest() {
    /**
     * The class to be tested.
     */
    @InjectMocks
    private lateinit var underTest: LongConversionProcessor
    ...
    fun testConvertByteShouldReturnProperResult() {
        ...
        // THEN
        Assertions.assertThat(actual).isEqualTo(LONG_VALUE)
    }

    @Test
    fun testConvertByteArrayShouldReturnProperResult() {
        // GIVEN
        val expected = ByteBuffer.wrap(EIGHT_BYTE_BYTE_ARRAY).long
    ...
    /**
     * Tests that the method `convertBoolean` returns the expected long.
     ...
     */
    @DataProvider
    private fun booleanToLongConvertValueTesting(): Array<Array<Any>> {
        return arrayOf(
            arrayOf(
                "Tests that the method returns 1 if the value is true",
                BOOLEAN_VALUE,
                TRUE_AS_LONG
            ), arrayOf("Tests that the method returns 0 if the value is false", false, FALSE_AS_LONG)
        )
    }
    ...
    companion object {
        private const val TRUE_AS_LONG = 1L
        private const val FALSE_AS_LONG = 0L
    }
}
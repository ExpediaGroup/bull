...
/**
 * Unit test for [ShortConversionProcessor].
 */
class ShortConversionTest : AbstractConversionTest() {
    /**
     * The class to be tested.
     */
    @InjectMocks
    private val underTest: ShortConversionProcessor? = null
    ...
    fun testConvertByteShouldReturnProperResult() {
        ...
        // THEN
        Assertions.assertThat(actual).isEqualTo(SHORT_VALUE)
    }
    ...
    fun testConvertByteArrayShouldReturnProperResult() {
        // GIVEN
        val expected = ByteBuffer.wrap(EIGHT_BYTE_BYTE_ARRAY).short
    ...
    fun testConvertCharacterShouldReturnProperResult() {
        ...
        Assertions.assertThat(actual)
            .isEqualTo(Character.getNumericValue(CHAR_VALUE).toShort())
    }

    /**
     * Tests that the method `convertBoolean` returns the expected short.
     ...
     */
    @DataProvider
    private fun booleanToShortConvertValueTesting(): Array<Array<Any>> {
        return arrayOf(
            arrayOf(
            ...
            arrayOf("Tests that the method returns 0 if the value is false", java.lang.Boolean.FALSE, FALSE_AS_SHORT)
        )
    }
    ...
    companion object {
        private const val TRUE_AS_SHORT: Short = 1
        private const val FALSE_AS_SHORT: Short = 0
    }
}
